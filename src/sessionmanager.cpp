...
#define CFG_KEY_FILE_INFO           "GCoderInfo"
#define CFG_KEY_FILE_INFO_TYPE      "Type"
...
{
    m_sessionsLimit = DEFAULT_SESSION_LIMIT;
    m_filesLimit = DEFAULT_FILES_LIMIT;
    SessionItem item;
...
{
    if (lim < 1 || lim > MAX_SESSION_LIMIT) {
        // Out of range.
        return;
    }
...
bool SessionManager::addFile(const QString &filePath)
{
    SessionItem &item = m_sessions[0];

    if (item.index(filePath) >= 0) {
...
bool SessionManager::documentInfo(GCoderInfo &info) const
{
...
bool SessionManager::setDocumentInfo(const GCoderInfo &info)
{
    SessionItem &item = m_sessions[0];
    int index = item.index(info.filePath);
...
QList<GCoderInfo> SessionManager::documentInfoList() const
{
    return m_sessions.at(0).files;
}

void SessionManager::setDocumentInfoList(const QList<GCoderInfo> &infoList)
{
    SessionItem &item = m_sessions[0];
    item.files = infoList;
...
void SessionManager::load(QSettings *cfg)
{
    ...
    if (m_sessionsLimit <= 0 || m_sessionsLimit > MAX_SESSION_LIMIT) {
        m_sessionsLimit = MAX_SESSION_LIMIT;
    }

    m_filesLimit = cfg->value(CFG_KEY_FILES_LIMIT, m_filesLimit).toInt();

    if (m_filesLimit <= 0 || m_filesLimit > MAX_FILES_LIMIT) {
        m_filesLimit = MAX_FILES_LIMIT;
    }

    QList<SessionItem> newSessions;

    for (int i = 1; i <= MAX_SESSION_LIMIT; i++) {
        ...
        cfg->beginGroup(CFG_KEY_FILE_INFO);
        item.files.clear();

        for (int j = 1; j <= MAX_FILES_LIMIT; j++) {
            cfg->beginGroup(QString::number(j));

            if (!cfg->contains(CFG_KEY_FILE_INFO_TYPE)) {
                cfg->endGroup();
                break;
            }

            GCoderInfo info;
            info.load(cfg, GCoderInfo());
            item.files.append(info);
            cfg->endGroup();
        }

        newSessions.append(item);
        cfg->endGroup(); // CFG_KEY_FILE_INFO
...
void SessionManager::save(QSettings *cfg) const
{
    ...
    for (const SessionItem &item : m_sessions) {
        ...
        for (const GCoderInfo &info : item.files) {
...
int SessionManager::SessionItem::add(const QString &filePath, int lim)
{
    int ind = index(filePath);
