                         ...
                         Qt::WindowFlags f) : QDialog(parent, f)
{
    ...
    QStringListIterator extIterator(editProp.editorProperties.extensions);

    while (extIterator.hasNext()) {
        lstExtensions->addItem(extIterator.next().toLocal8Bit().constData());
    }

    edtSaveExtension->setText(editProp.editorProperties.saveExtension);
    edtSaveDirectory->setText(editProp.editorProperties.saveDirectory);
...
AppConfig SetupDialog::getSettings()
{
    ...
    editProp.editorProperties.extensions.clear();

    for (int row = 0; row < lstExtensions->count(); row++) {
        QListWidgetItem *item = lstExtensions->item(row);
        editProp.editorProperties.extensions.append(item->text());
    }

    editProp.editorProperties.saveExtension = edtSaveExtension->text();
    editProp.editorProperties.saveDirectory = edtSaveDirectory->text();
